...
# Scene Flow:
def GetCorrelationGradientWeights(image1, mask1, image2, mask2, smoothingSigma = 1.0):
    beta_square = 10 # from paper page 14
    maskInt = np.multiply(mask1, mask2)
    image1 = np.multiply(image1, maskInt)
    image2 = np.multiply(image2, maskInt)
...
def ComputeFullSimilarityTerm(grad2M, gradIjX, gradIjY, JacobianJ, d, zi, N):
